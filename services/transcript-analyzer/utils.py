...
def setup_rabbit_entities(channel):
    ...
    channel.queue_declare(queue="dlq_transcript_analyzer", durable=True)
    channel.queue_bind(
        queue="dlq_transcript_analyzer",
        exchange="dead_letter_exchange",
        routing_key="transcript.analysis.failed",
    )
    ...
    arguments = {
        ...
        "x-dead-letter-routing-key": "transcript.analysis.failed",
    }
    channel.queue_declare(
        queue="transcript_analysis_queue",
        durable=True,
        arguments=arguments,
    )
    channel.queue_bind(
